 ...
 * @license MIT
 */
class TexylaPresenter extends BaseAdminPresenter
{
	/** @var string */
	...
	{
		parent::startup();
        $texyProcessor = new TexyProcessor(new Highlighter(new FshlHtmlOutput(),Highlighter::OPTION_DEFAULT),$this->configurator);
        $this->texy = $texyProcessor;
        $texy = $texyProcessor->getTexy();
        $parameters = $this->context->getParameters();
		$this->baseFolderPath = $texy->imageModule->fileRoot;
		$this->baseFolderUri = $texy->imageModule->root;
		$this->tempDir = $parameters['wwwDir'].'/webtemp/';
    ...
    public function actionPreview()
    {
        ...
        $html = Strings::trim($texy->process($post));
